...
def _generate_image_description(pil_img, caption: str = "", page_no: int = None) -> str:
    ...
    This description becomes the `content` field for image chunks — the primary
    text used for semantic search and retrieval. A good description dramatically
    improves retrieval quality over using a bare caption or placeholder.
...
1. Image type (chart / table / diagram / figure / photo)
...
def parse_document(file_path: str) -> list[dict]:
    ...
    for item in doc.iterate_items():
        ...
        # the Gemini Vision LLM can receive it directly during generation.
            ...
            # If a PIL image was successfully extracted, call Gemini Vision for
            # a rich, semantically meaningful description. This becomes the
            # `content` field — the primary field used for embedding and
            # semantic search retrieval.
            # Falls back to caption → placeholder if extraction failed.
